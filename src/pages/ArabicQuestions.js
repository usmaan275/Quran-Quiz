...
function ArabicQuestions() {
  ...
  const MIN_VISIBLE_CHARACTERS = 50;

  const stepForPtsDeducted = 50;
  ...
  //         const response = await axios.get(`https://api.alquran.cloud/v1/ayah/${currentAyah}/quran-indopak`);
  ...
  //       let starting = displayPassageWithBlanks(passageText);
  ...
  //       console.log(cleanText(starting + ending));
  //       setPassage(generateInitialDisplayJSX(starting, passageAyatRef.current, indexRef.current));
  //       }
  //     }
  //   } catch (err) {
  //     setError('Failed to fetch passage.');
  //     console.error('Error fetching passage:', err); // Log full error to console
  //     setPassage('');
  //     setPassageAyat([]);
  //     passageAyatRef.current = [];
  //   } finally {
  ...
  const fetchPassage = async () => {
    ...
    while (!isPassageFound) {
      ...
      while (passageText.length < MIN_VISIBLE_CHARACTERS) {
        ...
        const ayahText = ayahObject.text;
  ...
  const blankAyah = (ayahText) => {
    let string = ayahText.replace(/[ابتثجحخدذرزسشصضطظعغفقكلمنهويءأإآةؤئى]/g, '.');
    string = string.replace(/[ًٌٍَُِّْٰ]/g, '');
    return string;
  }

  const generateInitialDisplayJSX = (starting, passageAyatRef, indexRef) => {
    const passageJSX = [];
  
    // Add the initial `starting` part
    passageJSX.push(
      <span key="starting" style={{ color: 'white' }}>
        {starting}
      </span>
    );
  
    const startIndex = indexRef;
  
    for (let index = startIndex; index < passageAyatRef.length; index++) {
      const blankedAyah = blankAyah(passageAyatRef[index].text);
      passageJSX.push(
        <span key={index} style={{ color: index === startIndex ? 'turquoise' : 'white' }}>
          {blankedAyah}{'۝ '}
        </span>
      );
    }
  ...
  const displayPassageWithBlanks = (passageText) => {
    let start = ''; // The text we will build to show
    let remainingText = ''; // The remaining blanked-out passage
    let passageWords = passageText.split(' '); // Split passage into words

    let index = 0; // Word index to keep track of which word we are adding

    // Build the visible portion (start) by adding words until we reach MIN_VISIBLE_CHARACTERS
    while (index < passageWords.length) {
      let nextWord = passageWords[index]; // The next word in the passage
      start += nextWord + ' ';  // Add word with a space
      index++; // Move to the next word
    
      // Stop as soon as we hit or exceed the minimum character requirement
      if (start.length >= MIN_VISIBLE_CHARACTERS) {
        break;
      }
    ...
    // Remove trailing space from the final start string
    start = start.trim();    

    // Now, create the remaining blank part of the passage
    // Get the remaining words after we've built the 'start' string
    remainingText = passageWords.slice(index).join(' '); 

    // Replace each character in the remaining text with dots, except for spaces, diacretics and the '۝' symbol
    remainingText = remainingText.replace(/[ابتثجحخدذرزسشصضطظعغفقكلمنهويءأإآةؤئى]/g, '.');
    remainingText = remainingText.replace(/[ًٌٍَُِّْٰ]/g, '');

    // Concatenate the visible portion and the blanked-out text
    return start.trim() + ' ' + remainingText;  // Add ellipsis for blanked-out part
  }
  ...
  const handleUserSelection = (selectedAyahNumber) => {
    ...
    if (isDirectMatch || isMutashabih) {
      ...
      setNotification('Correct! Now what\'s the next ayat? (+10 points)');
      ...
      if (attemptsRef.current > 1) {
        ...
        setNotification('Incorrect! Try the next ayat. (-50 points)');
        setShowNotification(true);
  ...
  //     suggestionsElement.innerHTML = `<li class="no-results">جارٍ البحث...</li>`;
  //     suggestionsElement.classList.remove("hidden");
  
  //     // Fetch results from the Quran API
  //     const response = await fetch(`http://api.alquran.cloud/v1/search/${input}/all/quran-simple-clean`);
  //     if (response.ok) {
  ...
  //         // Fetch and display the Indo-Pak versions of the ayats
  //         const ayahDetails = await Promise.all(
  //           ayahNumbers.map(async (ayahNumber) => {
  //             try {
  //               const ayahResponse = await fetch(`http://api.alquran.cloud/v1/ayah/${ayahNumber}/quran-indopak`);
  //               if (ayahResponse.ok) {
  //                 const ayahData = await ayahResponse.json();
  //                 const ayahText = ayahData.data.text; // Indo-Pak text
  //                 const surahName = ayahData.data.surah.name; // Arabic Surah name
  ...
  //           suggestionsElement.innerHTML += `
  //             <li class="no-results">
  //               أظهرنا أول ١٠ نتائج، لنتائج أكثر ابحث في الصفحة المخصصة للبحث
  //             </li>`;
  //         }

  //       } else {
  //         suggestionsElement.innerHTML = `<li class="no-results">لم يتم العثور على نتائج</li>`;
  //       }
  //     } else {
  //       suggestionsElement.innerHTML = `<li class="no-results">حدث خطأ أثناء البحث</li>`;
  //     }

  //   } catch (error) {
  //     document.getElementById("suggestions").innerHTML = `<li class="no-results">حدث خطأ أثناء البحث</li>`;
  //   }
  // };

  const transliterateToArabic = (input) => {
    const map = {
      "a": "ا",
      "b": "ب",
      "t": "ت",
  ...
  function sanitizeInput(input) {
    ...
    return input.replace(/[^\p{Script=Arabic}\p{Punctuation}\p{Zs}]/gu, '');
  }    
...
const handleSearch = (input) => {
    ...
    matches.forEach((match) => {
      ...
      if (!inGroup || isFirstInGroup) {
        if (listCount < 10) {
          ...
          const cleanedSurahName = cleanText(match.surah);
          listItem.innerHTML = `
            <p>${match.text}</p>
            <p style="color: gray; font-size: smaller;">${cleanedSurahName}</p>
          `;
  ...
  //   output = output.replace(/ اقسم/g, " أقسم");
  //   output = output.replace(/ ان/g, " إن");
  //   output = output.replace(/ فان/g, " فإن");
  //   output = output.replace(/ وان/g, " وإن");
  //   output = output.replace(/ فتنه/g, " فتنة");
  //   output = output.replace(/ ايات/g, " آيات");
  //   output = output.replace(/ ايها/g, " أيها");
  //   output = output.replace(/ سال/g, " سأل");
  //   output = output.replace(/ انت/g, " أنت");
  //   output = output.replace(/ الانسان/g, " الإنسان");
  //   output = output.replace(/ انسان/g, " إنسان");
  //   output = output.replace(/الارض/g, "الأرض");
  //   output = output.replace(/ اهلك/g, " أهلك");
  //   output = output.replace(/ انج/g, " أنج");
  //   output = output.replace(/ قران/g, " قرآن");
  //   output = output.replace(/ اخوان/g, " إخوان");
  //   output = output.replace(/ اباء/g, " آباء");
  //   output = output.replace(/ امه/g, " أمة");
  //   output = output.replace(/ اصلح/g, " أصلح");
  //   output = output.replace(/الا /g, "إلا ");
  //   output = output.replace(/ راي/g, " رأي");
  //   output = output.replace(/اذ /g, "إذ ");
  //   output = output.replace(/اذا /g, "إذا ");
  //   output = output.replace(/ الى/g, " إلى");
  //   output = output.replace(/ابراهيم/g, "إبراهيم");
  //   output = output.replace(/لملائكه/g, "لملائكة");
  //   output = output.replace(/امنوا/g, "آمنوا");
...
export default ArabicQuestions;
